 ...
 *
 * <p><b>Telemetry (ADR-0079 D5)</b>: {@link #replay} emits one {@code replay.run}
 * {@link io.ara.core.telemetry.Span} — {@code original_run_id}, {@code replay_run_id},
 * {@code overrides_count}, {@code cache_hit_ratio} (cache-hit occurrences over total —
 * quantifies the causal cone a substitution left untouched). No preexisting span on this
 * operation to extend; a new one, same as ADR-0078 D5's {@code memory.evict}/{@code
 * memory.recall}.
 */
public final class TraceReplayEngine implements ReplayEngine {
    ...
    public TraceReplayEngine(TraceStore traceStore, BlobStore blobStore, ReplayExecutor executor) {
        this(traceStore, blobStore, executor, AraTelemetry.noop());
    }
    ...
    public ReplayResult replay(String originalRunId, String replayRunId, Map<String, String> overrides) {
        ...
        Span span = telemetry.spanBuilder("replay.run")
                .setAttribute("original_run_id", originalRunId)
                .setAttribute("replay_run_id", replayRunId)
                .setAttribute("overrides_count", (long) subs.size())
                .startSpan();
        try (var scope = span.makeCurrent()) {
            ...
            for (TraceSpan occurrence : original) {
                String key = occurrence.spanId();
                if (subs.containsKey(key)) {
                    replayed.add(rebase(occurrence, replayRunId, subs.get(key), new SpanStatus.Completed(), null));
                    overridden++;
                } else if (occurrence.outputRef() != null && blobStore.get(occurrence.outputRef()).isPresent()) {
                    replayed.add(rebase(occurrence, replayRunId, occurrence.outputRef(), occurrence.status(),
                            occurrence.failureKind()));
                    hits++;
                } else {
                    String fresh = executor != null ? executor.reexecute(occurrence) : null;
                    if (fresh != null) {
                        replayed.add(rebase(occurrence, replayRunId, fresh, new SpanStatus.Completed(), null));
                    } else {
                        replayed.add(rebase(occurrence, replayRunId, null,
            ...
            ReplayResult result = new ReplayResult(replayRunId, replayed, hits, overridden, misses);
            span.setAttribute("cache_hit_ratio", result.cacheHitRatio())
                    .setStatus(io.ara.core.telemetry.SpanStatus.OK);
            return result;
        } catch (RuntimeException e) {
            span.recordException(e).setStatus(io.ara.core.telemetry.SpanStatus.ERROR);
            throw e;
        } finally {
            span.end();
        }
